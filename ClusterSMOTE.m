function [final_features ,final_mark] = ClusterSMOTE(original_features, original_mark, Ncluster,Num_Ov)
...
while j2 <= Num_Ov
    ...
    if size(Condidates,2)>= 1
        ...
        g = Min_cand(Sel_index,:);
        alpha = rand(1);
        snew = S2(1,:) + alpha.*(g-S2(1,:));
        final_features = [snew;final_features];
        j2=j2+1;
    end
end

mark = 1 * ones(Num_Ov,1);
final_mark = [mark;original_mark];